...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    auto temp = new std::pair<K, V>(key, value);
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     * @hint Use findPrime()!
     */
    std::pair<K, V> **temp = new std::pair<K, V>*[findPrime(size * 2)];
    delete[] should_probe;
    ...
    for (size_t i = 0; i < size; i++) {
        if (table[i]) {
            unsigned current = hashes::hash(table[i]->first, findPrime(size * 2));
            while (temp[current]) {
                current++;
                current %= findPrime(size * 2);
            }
            temp[current] = table[i];
