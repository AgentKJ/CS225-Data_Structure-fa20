                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.size() == 0) {
        smaller = mergeQueue.front();
        mergeQueue.pop();
    } else if (mergeQueue.size() == 0) {
        smaller = singleQueue.front();
        singleQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() != 1) {
        ...
        TreeNode *sum = new TreeNode((first->freq).getFrequency() + (second->freq).getFrequency());
        sum->left = first;
        sum->right = second;
        mergeQueue.push(sum);
        sum = NULL;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()) {
        if (bfile.getNextBit() == 1) {
            char next = bfile.getNextByte();
            toReturn = new TreeNode(Frequency(next, 0));
        } else {
            toReturn = new TreeNode(0);
            toReturn->left = readTree(bfile);
            toReturn->right = readTree(bfile);
        }
    }
    return toReturn;
}
