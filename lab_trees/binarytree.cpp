...
bool BinaryTree<T>::isOrderedIterative() const
{
    ...
    for (TreeTraversal<int>::Iterator iter = in.begin(); iter != in.end(); ++iter){
        if(temp == NULL){
            temp = *iter;
        }
        
        if(temp->elem > (*iter)->elem){
           return false;
        }
        if(temp->elem < (*iter)->elem){
            temp = *iter;
...
bool BinaryTree<T>::isOrderedRecursive(Node *subRoot, Node *&temp) const {
    if (subRoot != NULL) {
        if (!isOrderedRecursive(subRoot->left, temp)) {
            return false;
        }
        if (temp != NULL && subRoot->elem <= temp->elem) {
            return false;
        }
        temp = subRoot;
