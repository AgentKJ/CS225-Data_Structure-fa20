...
class SquareMaze {
     ...
     * 
     * If this object already represents a maze it will clear all the existing data before
     * doing so. You will start with a square grid (like graph paper) with the specified
     * height and width. You will select random walls to delete without creating a cycle,
     ...
     * 
     * This function should be fast (constant time). You can assume that in grading we will
     * not make your maze a non-tree and then call one of the other member functions. setWall
     * should not prevent cycles from occurring, but should simply set a wall to be present
     * or not present. Our tests will call setWall to copy a specific maze into your implementation.
     * 
     * @param x	The x coordinate of the current cell
     * @param y	The y coordinate of the current cell
     ...
     * exists, then blacken the pixels with coordinates ((x+1)*10,y*10+k) for k from 0 to 10.
     * If the bottom wall exists, then blacken the pixels with coordinates (x*10+k, (y+1)*10)
     * for k from 0 to 10.
     * 
     * The resulting PNG will look like the sample image, except there will be no exit from
     * the maze and the red line will be missing.
     ...
     * (5,5) through (5,15) red. (Red is 0,1,0.5,1 in HSLA). Then if the second step is
     * right, color pixels (5,15) through (15,15) red. Then if the third step is up, color
     * pixels (15,15) through (15,5) red. Continue in this manner until you get to the end
     * of the solution vector, so that your output looks analogous the above picture.
     * 
  ...
  private:
    ...
    vector<pair<bool, bool>> _walls; // <right, down>
