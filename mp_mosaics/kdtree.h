...
class KDTree
{
  ...
  private:
     ...
     */
    void buildTree(KDTreeNode* subroot, vector<Point<Dim>> newPoints, int curDim);
    vector<Point<Dim>> sort(vector<Point<Dim>> newPoints, int curDim);
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTree<Dim>::KDTreeNode *current, int curDim) const;
};
